...
class ModelWorker:
  def __init__(self, model_path='tencent/Hunyuan3D-2', device='cuda'):
    ...
    self.pipe = offload.extract_models("i23d_worker", self.pipeline)
  ...
  def generate(self, uid, params):
    if 'image' in params:
      image = Image.open(BytesIO(params["image"]))
    else:
      if 'text' in params:
        text = params["text"]
    ...
    if params.get('texture', False):
      ...
      mesh = FaceReducer()(mesh, max_facenum=params.get('face_count', params['face_count']))
...
async def generate(request: Request):
  ...
  uid = uuid.uuid4()
  try:
    file_path, uid = worker.generate(uid, { "image": image_bytes })
